function drawSearchlight(ctx, originX, originY, beamLength, spread, drift, tint) {
	ctx.save();
...
function drawPrisonComplex(ctx, canvas, groundY, scene) {
	const wallY = 315;
	...
	for (let tx = -240; tx < canvas.width + 240; tx += 240) {
		...
		drawSearchlight(ctx, towerX + 49, wallY - 92, 320, 70, sweep, "rgba(167, 219, 255, 0.26)");
	}
...
function drawPoliceLights(ctx, canvas, groundY, scene) {
...
export { drawPrisonComplex, drawPoliceLights, drawSearchlight };