...
export function applyLevel1Collisions(prisoner, scene, canvas, level, groundY, previousY) {
	...
	if (
		playerRect.x < finishRect.x + finishRect.w &&
		playerRect.x + playerRect.w > finishRect.x &&
		playerRect.y < finishRect.y + finishRect.h &&
		playerRect.y + playerRect.h > finishRect.y
	) {
		scene.levelWon = true;
		prisoner.vx = 0;
		prisoner.vy = 0;
